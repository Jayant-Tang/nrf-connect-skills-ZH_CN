...
2. **MCP 不通就停**：Nordic MCP 不可用时停下来让用户恢复并说明影响；**不得用 workaround（自写脚本、纯靠源码推断等）静默绕过**。用户明确同意放弃 MCP 后，才可改用源码独立推进。
...
5. **方向错就接力**：一旦发现理解错需求 / 方向错误 / 需返工，或上下文将满，先停下与用户确认，再 `Handover.md` + clear，**不在原会话里硬扛打补丁**。
...
2. Nordic MCP（**MCP不通时停下来要求用户恢复 / 重新认证，并说明影响；不要用 workaround 静默绕过**）。
...
2. **接力时机（满足任一即接力）**：上下文将满，**或发现返工级偏差（理解错需求 / 方向错误 / 需推翻重来）**。先停下与用户确认，再把进度 / 已完成 / 待办 / 坑写入当前工程 `Handover.md`，然后 clear；禁止修改原始 plan 文档（除非用户同意）。不要在原会话里靠不断打补丁掩盖方向错误。
